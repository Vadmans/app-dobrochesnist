...
.nav{display:grid;gap:8px}.nav button{width:100%;text-align:left;border:0;border-radius:14px;padding:12px 14px;color:rgba(255,255,255,.82);background:transparent;cursor:pointer;font-weight:600;font-size:14px;display:flex;align-items:center;gap:12px;transition:all 0.2s}.nav button:hover,.nav button.active{background:rgba(255,255,255,.14);color:#fff}.nav .ico{width:28px;height:28px;border-radius:10px;background:rgba(255,255,255,.10);display:inline-flex;align-items:center;justify-content:center;font-size:14px}.side-footer{position:absolute;left:20px;right:20px;bottom:24px;border-top:1px solid rgba(255,255,255,.15);padding-top:16px}.side-footer a{display:block;color:rgba(255,255,255,.8);text-decoration:none;margin-top:10px;font-weight:500;font-size:13px}
...
<div class="app">
  <aside class="sidebar">
    ...
    <div class="side-footer">
      <div style="color:rgba(255,255,255,.6);font-size:11px;margin-bottom:8px">Швидкі посилання</div>
...
async function loadRefs(show=false){
  try{
    ...
    tb.innerHTML='';
    if(!d.length){tb.innerHTML='<tr><td colspan="4"><div class="empty">Записів поки немає</div></td></tr>';return;}
    d.forEach(x=>{
      const tr=document.createElement('tr');
      tr.innerHTML=`
...
async function loadChat(show=false){
  try{
    ...
    tb.innerHTML='';
    if(!d.length){
      tb.innerHTML='<tr><td colspan="6"><div class="empty">💬 Питань поки немає</div></td></tr>';
