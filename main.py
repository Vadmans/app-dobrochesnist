...
# ── Пуш-повідомлення (Firebase Cloud Messaging) ───────────────────────────
# Усе захищено try/except: якщо firebase-admin не встановлено або немає ключа
# FIREBASE_CREDENTIALS — пуші просто не надсилаються, застосунок працює далі.
_fb_ready = False
try:
    ...
    _creds_raw = os.getenv("FIREBASE_CREDENTIALS", "")
    if _creds_raw:
        firebase_admin.initialize_app(credentials.Certificate(_json.loads(_creds_raw)))
        _fb_ready = True
except Exception:
    _fb_ready = False
...
def send_push_to_all(title: str, body: str):
    ...
    db = SessionLocal()
    try:
        tokens = [d.token for d in db.query(Device).all()]
    finally:
        db.close()
    if not tokens:
        return
...
def register_device(data: DeviceIn, db: Session = Depends(get_db)):
    """Застосунок реєструє тут свій push-токен (відкритий ендпоінт)."""
    if data.token and not db.get(Device, data.token):
        db.add(Device(token=data.token))
        db.commit()
...
def create_event(data: EventIn, db: Session = Depends(get_db)):
    ...
    db.commit()
    db.refresh(ev)
    send_push_to_all("Нова подія", f"{ev.title} — {ev.date.strftime('%d.%m.%Y')}")
    return ev
