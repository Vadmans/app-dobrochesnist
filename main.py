...
- модуль Заняття: YouTube прямі ефіри та записи.
...
class Lesson(Base):
    ...
    kind: Mapped[str] = mapped_column(String, default="record")  # live або record
            ...
            if DATABASE_URL.startswith("sqlite"):
                ...
                conn.execute(text("""
                    CREATE TABLE IF NOT EXISTS lessons (
                        ...
                        description TEXT DEFAULT '',
                        youtube_url TEXT DEFAULT '',
                        kind VARCHAR DEFAULT 'record',
                        lesson_date DATE,
                        created_at TIMESTAMP DEFAULT NOW()
                    )
                """))
                conn.execute(text("ALTER TABLE lessons ADD COLUMN IF NOT EXISTS description TEXT DEFAULT ''"))
                conn.execute(text("ALTER TABLE lessons ADD COLUMN IF NOT EXISTS youtube_url TEXT DEFAULT ''"))
                conn.execute(text("ALTER TABLE lessons ADD COLUMN IF NOT EXISTS kind VARCHAR DEFAULT 'record'"))
                conn.execute(text("ALTER TABLE lessons ADD COLUMN IF NOT EXISTS lesson_date DATE"))
                conn.execute(text("ALTER TABLE lessons ADD COLUMN IF NOT EXISTS created_at TIMESTAMP DEFAULT NOW()"))
...
class LessonIn(BaseModel):
    ...
    description: str = ""
    youtube_url: str = ""
    kind: str = "record"  # live або record
...
def stats(db: Session = Depends(get_db)):
    try:
        ...
        return JSONResponse(status_code=200, content={"ok": False, "error": str(e), "events": 0, "views": 0, "categories": 0, "devices": 0, "chat_new": 0, "reference": 0, "lessons": 0, "push_logs": 0, "by_event": []})
...
<section id="lessons" class="tab hidden"><div class="card"><h2>Заняття YouTube</h2><input type="hidden" id="lessonId"><div class="formgrid"><div><label>Назва заняття</label><input id="lessonTitle" placeholder="Наприклад: Прямий ефір щодо декларування"></div><div><label>Дата</label><input id="lessonDate" type="date"></div><div><label>Тип</label><select id="lessonKind"><option value="live">Прямий ефір</option><option value="record">Запис заняття</option></select></div></div><label>YouTube посилання</label><input id="lessonUrl" placeholder="https://www.youtube.com/watch?v=... або https://youtu.be/..."><label>Опис</label><textarea id="lessonDesc" rows="3" placeholder="Коротко про заняття"></textarea><div class="actions" style="margin-top:14px"><button class="btn green" id="saveLesson">Зберегти заняття</button><button class="btn gray" id="clearLesson">Очистити</button></div></div><div class="card"><div class="toolbar" style="justify-content:space-between"><h2>Список занять</h2><button class="btn gray" id="reloadLessons">Оновити</button></div><div id="lessonList"></div></div></section>
...
function clearLesson(){['lessonId','lessonTitle','lessonDate','lessonUrl','lessonDesc'].forEach(id=>$(id).value='');$('lessonKind').value='live'}function editLesson(x){$('lessonId').value=x.id;$('lessonTitle').value=x.title||'';$('lessonDate').value=x.lesson_date||'';$('lessonKind').value=x.kind||'record';$('lessonUrl').value=x.youtube_url||'';$('lessonDesc').value=x.description||'';msg('Заняття відкрито для редагування')}async function loadLessons(){try{let r=await fetch('/lessons');allLessons=await readJson(r);let box=$('lessonList');box.innerHTML='';if($('stLessons'))$('stLessons').textContent=allLessons.length;if(!allLessons.length){box.innerHTML='<div class="muted">Занять поки немає</div>';return}allLessons.forEach(x=>{let d=document.createElement('div');d.className='refitem';let type=x.kind==='live'?'🔴 Прямий ефір':'▶️ Запис';let b=document.createElement('b');b.textContent=type+' — '+(x.title||'');let p=document.createElement('div');p.textContent=x.description||'';let l=document.createElement('div');l.className='muted';l.textContent=[x.lesson_date||'',x.youtube_url||''].filter(Boolean).join(' • ');let a=document.createElement('div');a.className='actions';let eb=document.createElement('button');eb.className='btn orange';eb.textContent='Редагувати';eb.onclick=()=>editLesson(x);let ob=document.createElement('button');ob.className='btn';ob.textContent='Відкрити YouTube';ob.onclick=()=>window.open(x.youtube_url,'_blank');let db=document.createElement('button');db.className='btn red';db.textContent='Видалити';db.onclick=()=>deleteLesson(x.id);a.append(eb,ob,db);d.append(b,p,l,a);box.append(d)})}catch(e){msg('Помилка занять: '+e.message,false)}}async function saveLesson(){if(!tok()){msg('Спершу вкажи ADMIN_TOKEN',false);return}let id=$('lessonId').value.trim(),payload={title:$('lessonTitle').value.trim(),description:$('lessonDesc').value.trim(),youtube_url:$('lessonUrl').value.trim(),kind:$('lessonKind').value,lesson_date:$('lessonDate').value||null};if(!payload.title||!payload.youtube_url){msg('Вкажи назву та YouTube посилання',false);return}try{let r=await fetch(id?`/lessons/${id}`:'/lessons',{method:id?'PUT':'POST',headers:h({'Content-Type':'application/json'}),body:JSON.stringify(payload)});let j=await readJson(r);if(!r.ok)throw new Error(j.detail||JSON.stringify(j));clearLesson();await loadLessons();await loadStats();msg(id?'Заняття оновлено':'Заняття додано')}catch(e){msg('Помилка заняття: '+e.message,false)}}async function deleteLesson(id){if(!tok()){msg('Спершу вкажи ADMIN_TOKEN',false);return}if(!confirm('Видалити заняття?'))return;let r=await fetch(`/lessons/${id}`,{method:'DELETE',headers:h()});if(r.ok||r.status===204){loadLessons();loadStats();msg('Заняття видалено')}else msg('Помилка видалення заняття',false)}
